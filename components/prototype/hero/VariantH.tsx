...
// 히어로 확정안 — 실제 스크린샷 데모카드 + 블루프린트.
// 폴리시 병합: ① 좌측 텍스트 stagger 진입(총 ~820ms) ② CTA press·focus-visible·hover 마감
// ③ 데모카드 1~3px pointer parallax + hover lift ④ 카드 hairline·hover elevation·느린 sheen
// 레이아웃·문구·카드 구성·crossfade 타이밍은 기존과 동일. reduced-motion 시 추가 모션 전부 제거.
...
import { DUR, EASE_OUT, EASE_OUT_CSS, DIST, STAGGER, HOVER_LIFT, PARALLAX } from "@/components/ui/motion-tokens";
...
/* ─── 데스크톱 실제 스크린샷 카드 — 그림자는 클래스(hover 전환), sheen 추가 ── */
...
/* ─────────────────────────────────────────────────────────────────────────────
   ...
   폴리시: 블루프린트/카드 레이어에 pointer parallax + hover lift (rAF lerp, state 없음)
   ───────────────────────────────────────────────────────────────────────────── */
function HeroAnimation({ reduceMotion }: { reduceMotion: boolean }) {
  ...
  // pointer parallax — target을 ref에만 저장, rAF에서 lerp (React state 미사용)
  useEffect(() => {
  ...
  return (
    <div ref={rootRef} className="polish-cards relative select-none" style={{ width: 540, height: 600, flexShrink: 0 }}>
      ...
      {/* ── DemoCard: z=10 — N장 항상 DOM에, 중앙 정렬, 동시 crossfade (parallax: 가까이 3px + hover lift) ── */}
...
/* ─────────────────────────────────────────────────────────────────────────────
   히어로 메인 — 레이아웃/문구 동일. 텍스트 stagger 진입 + CTA 마감
   ───────────────────────────────────────────────────────────────────────────── */
